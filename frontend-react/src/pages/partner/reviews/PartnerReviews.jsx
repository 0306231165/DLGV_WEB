...
const PartnerReviews = () => {
  // State lưu danh sách đánh giá từ API
  ...
  useEffect(() => {
    const fetchReviews = async () => {
      try {
        const response = await axiosClient.get('/nhan-vien/reviews');
        // axiosClient thường trả về data trực tiếp tùy thuộc cấu hình interceptor
  ...
  const averageRating = totalReviews > 0 ? (reviews.reduce((sum, item) => sum + item.rating, 0) / totalReviews).toFixed(1) : "0.0";

  const getStarCount = (star) => reviews.filter(r => r.rating === star).length;
                  ...
                  <div className="flex items-center justify-between mb-3 gap-2">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="w-10 h-10 rounded-xl bg-emerald-100 text-emerald-700 flex items-center justify-center font-black text-sm shrink-0">
                        {review.customer ? review.customer.replace('Chị ', '').replace('Anh ', '').replace('Cô ', '').replace('Chú ', '').charAt(0).toUpperCase() : 'K'}
                      </div>
                      <div>
