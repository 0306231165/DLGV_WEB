...
const ServiceDetailPage = () => {
    ...
    return (
      <div className="min-h-screen bg-surface flex flex-col items-center justify-center pt-20">
        ...
        <p className="text-on-surface-variant mb-6">
          Dịch vụ bạn đang tìm kiếm không tồn tại hoặc đã bị gỡ.
        </p>
        <button
  ...
  // Xác định tag hiển thị dựa trên mảng loai_goi (chấp nhận cả camelCase và snake_case)
  // Xác định tag hiển thị dựa trên số lượng loại gói
  // Lấy trực tiếp tên loại gói từ API và nối lại với nhau
  const getPackageTag = () => {
  // Phải dùng đúng tên thuộc tính là 'loai_goi' (snake_case)
  const loaiGoi = pkg.loai_goi || [];
  const count = loaiGoi.length;
  ...
  if (count > 0) {
    // Vì lg.loai_goi là 1 object chứa thông tin, nên phải lấy .ten_loai_goi từ đó
    return loaiGoi.map((lg) => lg.loai_goi?.ten_loai_goi).join(" / ");
  }

  return "Chưa có loại gói dịch vụ!";
};
          ...
          <div className="relative z-10 flex flex-col md:flex-row gap-8 items-start md:items-center">
            ...
            <div className="flex-grow">
              <div className="flex flex-wrap items-center gap-3 mb-2">
                <h1 className="font-h2 text-h2 font-bold text-on-surface">
                  {pkg.ten_dich_vu}
                </h1>
                {packageTag && (
              ...
              <div className="flex items-end gap-2 mt-4">
                <span className="text-sm text-on-surface-variant font-medium pb-1">
                  Giá chỉ từ
                </span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          <div className="md:col-span-2 space-y-8">
            ...
            <section className="bg-surface-container-lowest rounded-2xl p-6 md:p-8 border border-outline-variant/30 shadow-sm">
              ...
              {descriptions.map((paragraph, index) => (
                <p
                  key={index}
                  className="text-on-surface-variant leading-relaxed text-[15px] mb-4 last:mb-0"
                >
                    ...
                    <span
                      dangerouslySetInnerHTML={{
                        __html: paragraph.replace(
                          "Gói dịch vụ",
                          `Gói dịch vụ <strong>${pkg.ten_dich_vu}</strong>`,
                        ),
                      }}
                    />
                  ) : (
              ...
              <ul className="space-y-4">
                {tasks.map((task, i) => (
                  <li key={i} className="flex items-start gap-3">
                    ...
                    <span className="text-on-surface-variant text-[15px]">
                      {task}
                    </span>
                  </li>
                ))}
          ...
          <div className="md:col-span-1">
            <div className="sticky top-28 bg-surface-container-item rounded-2xl p-6 border border-primary/20 shadow-lg">
              <h3 className="font-h3 text-lg font-bold text-on-surface mb-4">
                Bạn đã sẵn sàng?
              </h3>
              <p className="text-sm text-on-surface-variant mb-6">
                Đặt lịch ngay hôm nay để tận hưởng không gian sạch sẽ, trong
                lành. Chúng tôi luôn sẵn sàng phục vụ!
              </p>
              <button
                onClick={() =>
                  navigate("/booking", { state: { selectedPackage: pkg.id } })
                }
