...
export default function RegisterPage() {
  ...
  // State quản lý trạng thái gửi dữ liệu (loading)
  ...
  // 4. Hàm xử lý gửi dữ liệu Đăng ký kết nối với API
  const handleRegisterSubmit = async (e) => {
    e.preventDefault();
    setErrorMessage(""); // Xóa thông báo lỗi cũ trước khi gửi request mới
    ...
    try {
      ...
      // GIẢI PHÁP PHÒNG THỦ: Lấy data lỗi từ Axios thuần HOẶC từ Interceptor đã bóc tách
      const errorData = error.response?.data || error;
      const status = error.response?.status;

      // Nếu có chứa object 'errors' từ Laravel validation (Status 422 hoặc có key errors)
      if (status === 422 || errorData?.errors) {
        const validationErrors = errorData.errors;

        // Lấy câu thông báo lỗi đầu tiên tìm thấy để hiển thị cho khách xem
        const firstErrorKey = Object.keys(validationErrors)[0];
        const firstErrorMessage = validationErrors[firstErrorKey][0];

        setErrorMessage(firstErrorMessage);
      } else {
        // Các lỗi hệ thống khác (500, mất mạng,...)
        const systemError =
  ...
  return (
    <div className="bg-surface text-on-surface font-body-md min-h-screen w-full overflow-x-hidden flex items-center justify-center relative py-12 md:py-20">
      {/* Block Style chứa các hiệu ứng loang và kính */}
      <style>{`
        .glass-morphism {
        ...
        <div className="flex flex-col md:flex-row w-full bg-white/40 rounded-[32px] overflow-hidden glass-morphism shadow-2xl">
          ...
          <div className="hidden md:flex md:w-1/2 relative min-h-[650px] flex-col justify-between p-12 bg-primary overflow-hidden">
            ...
            <div className="relative z-10">
              <h2 className="font-h2 text-h2 text-white mb-4 leading-snug">
                Chào mừng trở lại với CleanTrust
              </h2>
              ...
              <div className="flex items-center gap-4 p-4 rounded-2xl bg-white/10 backdrop-blur-md border border-white/20">
                <div className="w-12 h-12 rounded-full bg-primary-container flex items-center justify-center text-white">
                  ...
                  >
                    <path
                      fillRule="evenodd"
                      d="M8.603 3.799A4.49 4.49 0 0 1 12 2.25c1.357 0 2.573.6 3.397 1.549a4.49 4.49 0 0 1 3.498 1.307 4.491 4.491 0 0 1 1.307 3.497A4.49 4.49 0 0 1 21.75 12a4.49 4.49 0 0 1-1.549 3.397 4.491 4.491 0 0 1-1.307 3.497 4.491 4.491 0 0 1-3.497 1.307A4.49 4.49 0 0 1 12 21.75a4.49 4.49 0 0 1-3.397-1.549 4.49 4.49 0 0 1-3.498-1.306 4.491 4.491 0 0 1-1.307-3.498A4.49 4.49 0 0 1 2.25 12c0-1.357.6-2.573 1.549-3.397a4.49 4.49 0 0 1 1.307-3.497 4.49 4.49 0 0 1 3.497-1.307Zm7.007 6.387a.75.75 0 1 0-1.22-.872l-3.236 4.53L9.53 12.22a.75.75 0 0 0-1.06 1.06l2.5 2.5a.75.75 0 0 0 1.14-.1l3.75-5.25Z"
                      clipRule="evenodd"
                    />
                  </svg>
                </div>
                <div>
                  <p className="text-white font-label-sm font-semibold">
                    Hơn 5,000 khách hàng tin dùng
                  </p>
                  <p className="text-primary-fixed text-sm opacity-90">
                    Chất lượng dịch vụ 5 sao tại Việt Nam
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="w-full md:w-1/2 p-8 md:p-16 flex flex-col justify-center bg-white/60">
            <div className="mb-10 text-center md:text-left">
              <div className="text-primary font-h3 text-h3 font-bold mb-2 tracking-tight">
                CleanTrust
              </div>
              <h1 className="font-h2 text-h3 md:text-h2 text-on-surface mb-2">
                Đăng ký
              </h1>
            ...
            <form className="space-y-5" onSubmit={handleRegisterSubmit}>
              {/* KHỐI HIỂN THỊ LỖI THEO MẪU MỚI (TĨNH, KHÔNG NHẤP NHÁY) */}
              {errorMessage && (
              ...
              <div className="space-y-1">
                ...
                <div className="relative">
                  ...
                  <input
                    className="w-full pl-12 pr-4 py-3.5 rounded-xl border border-outline-variant bg-surface-container-lowest focus:ring-2 focus:ring-primary focus:border-transparent transition-all outline-none text-on-surface"
                    id="fullName"
                    type="text"
              ...
              {/* Ô Nhập Số điện thoại (BẮT BUỘC) */}
              <div className="space-y-1">
                <label className="block font-label-sm text-on-surface-variant" htmlFor="phone">
                  ...
                  <span className="absolute left-4 top-1/2 -translate-y-1/2 text-on-surface-variant/50">
                    ...
                    >
                      <path
                        strokeLinecap="round"
                        strokeLinejoin="round"
                        d="M10.5 1.5H8.25A2.25 2.25 0 0 0 6 3.75v16.5a2.25 2.25 0 0 0 2.25 2.25h7.5A2.25 2.25 0 0 0 18 20.25V3.75a2.25 2.25 0 0 0-2.25-2.25H13.5m-3 0V3h3V1.5m-3 0h3m-6 18.75h12"
                      />
                    </svg>
                  </span>
                  <input
              ...
              {/* Ô Nhập Email (TÙY CHỌN) */}
              <div className="space-y-1">
                <label className="block font-label-sm text-on-surface-variant" htmlFor="email">
                  Email <span className="text-on-surface-variant/40">(Không bắt buộc)</span>
                </label>
                <div className="relative">
                  <span className="absolute left-4 top-1/2 -translate-y-1/2 text-on-surface-variant/50">
                    <svg
                  ...
                  <span className="absolute left-4 top-1/2 -translate-y-1/2 text-on-surface-variant/50">
                    ...
                    >
                      <path
                        strokeLinecap="round"
                        strokeLinejoin="round"
                        d="M16.5 10.5V6.75a4.5 4.5 0 1 0-9 0V10.5m-2.25 0h13.5m-13.5 0a2.25 2.25 0 0 0-2.25 2.25v6.75a2.25 2.25 0 0 0 2.25 2.25h13.5a2.25 2.25 0 0 0 2.25-2.25v-6.75a2.25 2.25 0 0 0-2.25-2.25h-13.5m3 4.5h.008v.008H9.75v-.008ZM12 15h.008v.008H12V15Zm2.25 0h.008v.008h-.008V15Z"
                      />
                    </svg>
                  </span>
                  <input
                    className="w-full pl-12 pr-12 py-3.5 rounded-xl border border-outline-variant bg-surface-container-lowest focus:ring-2 focus:ring-primary focus:border-transparent transition-all outline-none text-on-surface"
                    id="password"
              ...
              <button
                type="submit"
                disabled={isLoading}
                className={`w-full bg-primary text-white font-h3 py-4 rounded-xl shadow-lg shadow-primary/20 hover:bg-on-primary-fixed-variant active:scale-[0.98] transition-all duration-300 font-semibold flex items-center justify-center gap-2 ${isLoading ? "opacity-70 cursor-not-allowed" : ""}`}
              >
                {isLoading ? (
                  <>
                    <svg
