...
export default function LoginPage() {
  ...
  const handleLoginSubmit = async (e) => {
    ...
    try {
      // authApi.loginKhachHang trả về response.data trực tiếp (axiosClient đã unwrap)
      // Shape: { message, token, role, user }
      const data = await authApi.loginKhachHang({
        so_dien_thoai: soPhone,
        mat_khau: password,
      });
    ...
    <div className="bg-surface text-on-surface font-body-md min-h-screen w-screen flex items-center justify-center relative py-10 overflow-y-auto">
      <style>{`
        ...
        .organic-shape-1 {
          ...
          position: absolute; top: -10%; right: -10%; z-index: 1;
            ...
            <form className="space-y-6" onSubmit={handleLoginSubmit}>
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    value={soPhone}
                    onChange={(e) => setSoPhone(e.target.value)}
                    disabled={loading}
                    required
                    className="w-full pl-12 pr-4 py-4 rounded-xl border border-outline-variant bg-surface-container-lowest focus:ring-2 focus:ring-primary focus:border-transparent transition-all outline-none text-on-surface disabled:opacity-60"
                  />
                </div>
              </div>
              ...
              <div className="space-y-2">
                <div className="flex justify-between items-center">
                  ...
                  <Link
                    to="/forgot-password"
                    className="text-primary font-label-sm hover:underline font-medium"
                  >
                    Quên mật khẩu?
                  </Link>
                </div>
                <div className="relative">
                  ...
                  <input
                    ...
                    disabled={loading}
                    required
                    className="w-full pl-12 pr-12 py-4 rounded-xl border border-outline-variant bg-surface-container-lowest focus:ring-2 focus:ring-primary focus:border-transparent transition-all outline-none text-on-surface disabled:opacity-60"
                  />
                  <button
                    type="button"
              ...
              <button
                type="submit"
                disabled={loading}
                className="w-full bg-primary text-white font-h3 py-4 rounded-xl shadow-lg shadow-primary/20 hover:bg-on-primary-fixed-variant active:scale-[0.98] transition-all duration-300 font-semibold disabled:opacity-60 disabled:cursor-not-allowed disabled:active:scale-100 flex items-center justify-center gap-2"
              >
                {loading ? (
                  <>
                    {/* Spinner */}
