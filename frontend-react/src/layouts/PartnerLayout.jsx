...
const PartnerLayout = () => {
  ...
  const menuItems = [
    ...
    {
      path: "/partner/skills-registration",
      icon: "model_training",
        ...
        <header className="h-20 bg-white border-b border-slate-200/60 fixed top-0 right-0 left-72 z-20 flex items-center justify-between px-8 shadow-sm">
          ...
          <div className="flex items-center gap-5">
            ...
            <div className="relative group/noti py-2">
              <button className="relative w-10 h-10 flex items-center justify-center rounded-full border transition-all focus:outline-none shadow-sm bg-slate-50 text-slate-600 group-hover/noti:bg-emerald-50 group-hover/noti:text-emerald-600 group-hover/noti:border-emerald-300 group-hover/noti:shadow-inner border-slate-200/60 cursor-pointer">
                ...
                <div className="p-4 bg-gradient-to-b from-slate-50 to-white border-b border-slate-100 flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-[10px] text-slate-400 font-medium">
                      Bạn có {unreadNotificationsCount} tin chưa đọc
                    </p>
                  </div>
                  {unreadNotificationsCount > 0 && (
                    <button
                  ...
                  {filteredNotifications.length > 0 ? (
                    filteredNotifications.map((noti) => (
                      ...
                      >
                        ...
                        <div
                          className={`w-9 h-9 rounded-xl flex items-center justify-center shrink-0 border border-black/5 ${noti.bgIcon}`}
                        >
                        ...
                        <div className="flex-1 space-y-0.5 text-left">
                          <div className="flex items-center justify-between gap-1">
                            <h4
                              className={`text-xs text-slate-800 line-clamp-1 ${!noti.isRead ? "font-black text-emerald-950" : "font-bold"}`}
                            >
                              {noti.title}
                            </h4>
