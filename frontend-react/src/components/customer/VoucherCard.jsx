  ...
  return (
    ...
    >
      ...
      >
        <span
          className={`text-wrap font-black leading-tight ${
            voucher.discountValue.length > 6
              ? "text-lg"
              : voucher.discountValue.length > 4
                ? "text-2xl"
                : "text-3xl"
          }`}
        >
        ...
        <span className="text-xs font-semibold mt-1 tracking-wider">
        ...
        <div className="absolute right-[-4px] top-0 bottom-0 w-[8px] flex flex-col justify-between py-1 z-10">
          {[...Array(8)].map((_, i) => (
            <div
              key={i}
              className="w-2 h-2 rounded-full bg-surface-container-lowest"
            ></div>
          ))}
        </div>
      </div>
      ...
      <div className="flex-1 p-5 flex flex-col relative bg-white dark:bg-surface-container">
        ...
        <p className="text-[13px] text-on-surface-variant flex-1 mb-4 leading-snug">
          {voucher.description}
        </p>
        ...
        <div className="flex justify-between items-end mt-auto pt-2 border-t border-outline-variant/20 border-dashed">
          <div className="flex flex-col">
            <span
              className={`text-[11px] ${isExpired ? "text-error font-semibold" : "text-on-surface-variant"}`}
            >
